"""
Pure-Python fallback for the removed `audioop` built-in module (PEP 594).

Adapted from pydub's bundled pyaudioop.py with Python 3 compatibility fixes:
- Replaced Python 2-only `buffer()` with direct bytes slicing
- Replaced `samples.next()` with `next(samples)`
- Voseo'd docstrings because VOICER is bilingual
"""
try:
...
def _get_sample(cp, size, i, signed=True):
    ...
    start = i * size
    return struct.unpack_from(fmt, cp, start)[0]
...
def ratecv(cp, size, nchannels, inrate, outrate, state, weightA=1, weightB=0):
    ...
    ceiling = (q + 1) * outrate
    nbytes = int(ceiling * bytes_per_frame)
